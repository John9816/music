...
class PlaylistCategoryChipAdapter(
    private val showGroupName: Boolean = false,
    ...
    var selectedApiName: String? = null
        set(value) {
            ...
            field = value
            previous?.let { old ->
                currentList.indexOfFirst { it.apiName == old }
                    .takeIf { it >= 0 }
                    ?.let(::notifyItemChanged)
            }
            value?.let { selected ->
                currentList.indexOfFirst { it.apiName == selected }
                    .takeIf { it >= 0 }
                    ?.let(::notifyItemChanged)
            }
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        ...
        return VH(binding, showGroupName, onClick)
    }
